...
fn bad_decode() {
    ...
    fn new<S>(name: &'static str, input: S) -> Test
    where
        S: Into<String>,
    {
        Test {
    ...
    let test_vector = vec![
        new("too few trailing dashes", *PEM_TOO_FEW_ENDING_DASHES),
        new("too many trailing dashes", *PEM_TOO_MANY_ENDING_DASHES),
        new("trailing non-whitespace", *PEM_TRAILING_NON_WHITESPACE),
        new("incorrect ending type", *PEM_WRONG_ENDING_TYPE),
        new("missing ending space", *PEM_MISSING_ENDING_SPACE),
        new("repeating begin", PEM_REPEATING_BEGIN.as_str()),
        new("missing end line", *PEM_MISSING_END_LINE),
    ];

    for test in test_vector.iter() {
        let rest = pem::decode(test.input.as_bytes())
            .map_err(|v| unsafe { str::from_utf8_unchecked(v) })
            .expect_err(&format!("decode {}", test.name));
        assert_eq!(test.input, rest, "{}, bad rest", test.name);
    }
}

/// ref: https://github.com/advisories/GHSA-q42m-q8hq-53rj
#[test]
fn cve202224675() {
    // Prior to CVE-2022-24675, this input would cause a stack overflow.
    let input = "-----BEGIN \n".repeat(10000000);
    let rest = pem::decode(input.as_bytes()).unwrap_err();
    //panic!("hello");
    assert_eq!(input.as_bytes(), rest);
}

#[test]
fn decode() {
    let (result, remainder) = pem::decode(PEM_DATA.as_bytes()).unwrap();
    assert_eq!(*CERTIFICATE, result, "#0: decode cert");

    let (result, remainder) = pem::decode(remainder).unwrap();
    assert_eq!(*PRIVATE_KEY, result, "#1: decode private key");

    fn is_empty(block: &Block) -> bool {
        (block.type_ == "EMPTY") && block.headers.is_empty() && block.bytes.is_empty()
    }

    let (result, remainder) = pem::decode(remainder).unwrap();
    assert!(
        is_empty(&result),
        "#2 should be empty but got: {:?}",
        result
    );
    ...
    assert!(
        is_empty(&result),
        "#4 should be empty but got: {:?}",
        result
    );

    let (result, remainder) = pem::decode(remainder).unwrap();
    assert_eq!(
        "HEADERS", result.type_,
        "#5 expected single header block: bad type"
    );
    assert_eq!(
        1,
        result.headers.len(),
        "#5 expected single header block: bad #(headers)"
    );

    assert_eq!(
        0,
        remainder.len(),
        "expected nothing remaining of PEM_DATA, but found {}",
        unsafe { std::str::from_utf8_unchecked(remainder) }
    );

    let (result, _) = pem::decode(testbot::PEM_PRIVATE_KEY2.as_bytes()).unwrap();
    assert_eq!(*testbot::PRIVATE_KEY2, result, "#6");
}

lazy_static::lazy_static! {
  static ref CERTIFICATE: Block = Block{
    type_: "CERTIFICATE".to_string(),
    bytes: vec![
        0x30, 0x82, 0x3, 0xe9, 0x30, 0x82, 0x3, 0x52, 0x2, 0x1,
  ...
  static ref PEM_DATA: String = {
    const RAW:&'static str = r#"verify return:0
-----BEGIN CERTIFICATE-----
sdlfkjskldfj
  -----BEGIN CERTIFICATE-----
---
...
-----END HEADERS-----"#;
    ...
    testbot::testing_key(RAW)
  };

  static ref PEM_MISSING_ENDING_SPACE: &'static str = r#"
-----BEGIN FOO-----
dGVzdA==
-----ENDBAR-----"#;

static ref PEM_MISSING_END_LINE: &'static str = r#"
-----BEGIN FOO-----
Header: 1"#;


  static ref PEM_REPEATING_BEGIN: String = "-----BEGIN \n".repeat(10);

  static ref PEM_TOO_FEW_ENDING_DASHES: &'static str = r#"
-----BEGIN FOO-----
dGVzdA==
-----END FOO----"#;

static ref PEM_TOO_MANY_ENDING_DASHES: &'static str = r#"
-----BEGIN FOO-----
dGVzdA==
-----END FOO------"#;

static ref PEM_TRAILING_NON_WHITESPACE: &'static str = r#"
-----BEGIN FOO-----
dGVzdA==
-----END FOO----- ."#;

static ref  PEM_WRONG_ENDING_TYPE: &'static str = r#"
-----BEGIN FOO-----
dGVzdA==
-----END BAR-----"#;

  static ref PRIVATE_KEY: Block = {
    let type_ = "RSA PRIVATE KEY".to_string();

    let mut headers:HashMap<String, String> = HashMap::default();
    headers.insert("DEK-Info".to_string(), "DES-EDE3-CBC,80C7C7A09690757A".to_string());
    headers.insert("Proc-Type".to_string(), "4,ENCRYPTED".to_string());

    Block { type_, headers, bytes: vec![
      0x79, 0xa, 0x79, 0x66, 0x41, 0xfa, 0xb,
